...
export const fetchRestaurants = async (
  ...
  if (type === AppTab.GOURMET) {
    ...
    if (isExpandedSearch) {
      ...
      3. TU OBJETIVO: Buscar en el OFFICIAL_REGISTRY restaurantes que estén situados en "${location}" O en cualquiera de esos pueblos vecinos identificados.
      `;
    } else {
      geoLogicInstruction = `
      FASE 1 - LÓGICA GEOGRÁFICA (ESTRICTA):
      1. Identifica el municipio solicitado: "${location}".
      2. TU OBJETIVO: Buscar en el OFFICIAL_REGISTRY ÚNICAMENTE restaurantes cuyo municipio coincida exactamente con "${location}". Ignora alrededores.
      `;
    }

    systemInstruction = `Actúa como una API de doble motor: Motor de Base de Datos Estricta + Motor de Conocimiento Gastronómico.
    
    TIENES PROHIBIDO INVENTAR RESTAURANTES. Solo puedes devolver restaurantes que existan físicamente en el texto del OFFICIAL_REGISTRY proporcionado abajo.
    ...
    FORMATO DE SALIDA:
    Devuelve un JSON Array limpio.
    `;

    prompt = `Ubicación Usuario: "${location}". Radio: "${distance}".
    
    OFFICIAL_REGISTRY (Fuente de Verdad Única):
    ---
    ${OFFICIAL_REGISTRY}
    ---
